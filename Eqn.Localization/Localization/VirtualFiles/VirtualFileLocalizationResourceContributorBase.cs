using Eqn.Core;
using Eqn.Core.Internal;
using Eqn.Core.System;
using Eqn.Core.System.Collections.Generic;
using Eqn.VirtualFileSysem.Extensions.FileProviders;
...
namespace Eqn.Localization.Localization.VirtualFiles;
...
public abstract class VirtualFileLocalizationResourceContributorBase : ILocalizationResourceContributor
{
    ...
    private Dictionary<string, ILocalizationDictionary> CreateDictionaries()
    {
        ...
        foreach (var file in _virtualFileProvider.GetDirectoryContents(_virtualPath))
        {
            ...
            {
                throw new EqnException($"{file.GetVirtualOrPhysicalPathOrNull()} dictionary has a culture name '{dictionary.CultureName}' which is already defined! Localization resource: {_resource.ResourceName}");
            }
