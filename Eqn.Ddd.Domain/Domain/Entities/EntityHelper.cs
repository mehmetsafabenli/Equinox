...
namespace Eqn.Ddd.Domain.Domain.Entities;
...
public static class EntityHelper
{
    ...
    public static void CheckEntity([NotNull] Type type)
    {
        ...
        if (!IsEntity(type))
        {
            throw new EqnException($"Given {nameof(type)} is not an entity: {type.AssemblyQualifiedName}. It must implement {typeof(IEntity).AssemblyQualifiedName}.");
        }
    }
